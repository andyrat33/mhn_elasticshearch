...
def iterate_distinct_field(es, fieldname, pagesize=250, **kwargs):
    ...
    compositeQuery = {
        "size": pagesize,
        "sources": [{fieldname: {"terms": {"field": fieldname}}}],
    }
    # Iterate over pages
    while True:
        result = es.search(
            **kwargs, body={"aggs": {"values": {"composite": compositeQuery}}}
        )
        # Yield each bucket
        ...
        if "after_key" in result["aggregations"]["values"]:
            compositeQuery["after"] = result["aggregations"]["values"]["after_key"]
        else:  # Finished!
            break
...
parser = argparse.ArgumentParser(
    description="Get Passwords/Threat Intel from Modern Honey Net Elasticsearch and store to a named file"
)
parser.add_argument(
    "-e",
    dest="mhn_address",
    action="store",
    help="MHN Elasticsearch host Address",
    default="mhn",
)
parser.add_argument(
    "-p",
    help="MHN Port (default 9200)",
    default=9200,
    type=int,
    action="store",
    dest="mhn_port",
